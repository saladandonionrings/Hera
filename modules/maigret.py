...
FALSE_POSITIVE_SITES = {
    ...
    "chatujme.cz",
    "reverbnation",
    # User-reported batch: every one of these came back as a bare
    # "Username: X" echo with zero corroborating info (no bio, avatar,
    # or anything site-specific) across ~35 completely unrelated
    # services (dating, Apple forums, art, hardware, wikis, an NFT
    # marketplace...) for the same username - the signature of a weak
    # status-code-only detection method in maigret's own site database,
    # not a real account match.
    "3ddd",
    "adultfriendfinder",
    "appledeveloper",
    "applediscussions",
